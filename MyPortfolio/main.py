...
def start(message):
    ...
    bot.send_message(message.chat.id,"please select language to continue",reply_markup=lan)
...
github = telebot.types.InlineKeyboardButton("گیت هاب",url="https://github.com/rezadrakhshan")
...
def persian(message):
    if message.text == "persian":
        msg = bot.send_message(message.chat.id,"گزینه خود را انتخاب کنید.",reply_markup=perchoices)
        bot.register_next_step_handler(msg, command)  
    elif message.text == "english":
        msg = bot.send_message(message.chat.id,"select your choice.",reply_markup=enchoices)
        bot.register_next_step_handler(msg , command)
