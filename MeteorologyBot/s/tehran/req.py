...
def tehrantemp(data):
    if data == "tehran":
        ...
        return f"""<b>وضعیت دما در شهر تهران :</b>
...
⚠️<i>توجه: این آمار برای امروز می‌باشد!!</i>
    """ 
    if data == "shahriar":
        URL = "https://api.open-meteo.com/v1/forecast?latitude=35.6588&longitude=51.0577&hourly=temperature_2m"
        location = "delhi technologicaluniversity"
        PARAM = {"adress": location}
        info = requests.get(url=URL, params=PARAM)
        data = info.json()
        temps = data["hourly"]["temperature_2m"]
        return f"""<b>وضعیت دما در شهر شهریار:</b>
 دما هم اکنون = <code>{temps[hour]}</code> درجه سانتی‌گراد
<b>دما در ساعت های مختلف:</b>
۰۰:۰۰ = {temps[0]}
۰۱:۰۰ = {temps[1]}
